  ...
  async getCsvLines(examId: number): Promise<CsvLine[]> {
    ...
    const csvLines: CsvLineRow[] = await queryRunner.query(
      ...
      SELECT DISTINCT 
        ...
        u.type AS 'user_type'
      FROM exam_agreement ea
        INNER JOIN USER u ON u.id = ea.user_id
        INNER JOIN answer a ON a.exam_agreement_id = ea.id
        JOIN grade g ON g.id = a.grade_id
        INNER JOIN question q ON q.id = a.question_id
        INNER JOIN question_group qg ON qg.id = q.group_id
      WHERE ea.exam_id = ?
        AND ea.deleted_at IS NULL
      ORDER BY ea.id ASC, qg.id ASC, q.id ASC
      `,
      [examId],
    );
